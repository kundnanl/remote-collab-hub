...
export function SprintList({ orgId }: { orgId: string }) {
  ...
  const { data: sprints = [], isLoading } = trpc.sprints.list.useQuery({ orgId });
  ...
  const start = trpc.sprints.start.useMutation({
    onMutate: async (vars) => {
      ...
      utils.sprints.list.setData({ orgId }, (old) =>
        old?.map((s) =>
          s.id === vars.sprintId ? { ...s, status: "ACTIVE" } : s
        ) ?? []
      );
      return { prev };
    },
  ...
  const close = trpc.sprints.close.useMutation({
    onMutate: async (vars) => {
      ...
      utils.sprints.list.setData({ orgId }, (old) =>
        old?.map((s) =>
          s.id === vars.sprintId ? { ...s, status: "CLOSED" } : s
        ) ?? []
      );
      return { prev };
    },
  ...
  return (
    <div className="space-y-4">
      ...
      <SprintDialog orgId={orgId} open={open} setOpen={setOpen} />
